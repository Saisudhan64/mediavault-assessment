...
export function AssetDetail({ id, onClose, onSaved }: Props) {
  ...
  try {
    ...
    if (err?.status === 409) {
      // asset was updated by someone else — refetch latest version
      setError('This asset was updated by someone else. Refreshing…');
    ...
    <aside className="panel">
      ...
      {asset && (
        <div className="panel__body">
          {asset.hasThumbnail ? (
            <img className="card__thumb" src={thumbnailUrl(asset.id)} alt={asset.name} />
          ) : (
